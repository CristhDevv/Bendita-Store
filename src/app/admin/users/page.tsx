...
export default function AdminUsersPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-display text-3xl text-crystal mb-1">Usuarios</h1>
        <p className="font-body text-sm text-crystal/50">{users.length} clientes registrados</p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        {[
          ...
          <div key={label} className="glass border border-white/5 rounded-xl p-4 flex items-center gap-3">
            <div className={`w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center ${color}`}>
              <Icon className="w-4 h-4" />
            </div>
            <div>
              <p className="font-display text-xl text-crystal">{value}</p>
              <p className="font-body text-xs text-crystal/40">{label}</p>
            </div>
          </div>
        ))}
      </div>
                  ...
                  >
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-gold-400/30 to-gold-600/10 flex items-center justify-center text-gold font-display font-bold text-sm shrink-0">
                          {(user.full_name || "?").charAt(0).toUpperCase()}
                        </div>
                        <div>
                          <p className="font-body text-sm text-crystal">
                            {user.full_name || <span className="text-crystal/30 italic">Sin nombre</span>}
                          </p>
                          <p className="font-body text-xs text-crystal/40">
                    ...
                    <td className="px-4 py-3">
                      {user.phone ? (
                        <div className="flex items-center gap-1.5 text-crystal/60">
                          <Mail className="w-3 h-3" />
                          <span className="font-body text-xs">{user.phone}</span>
                        </div>
                      ) : (
                        <span className="font-body text-xs text-crystal/20">—</span>
                      )}
                    </td>
                    <td className="px-4 py-3 font-body text-xs text-crystal/50">
                    ...
                    <td className="px-4 py-3">
                      {user.is_admin ? (
                        ...
                        <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-body border text-crystal/40 border-white/8 bg-white/3">
                          Cliente
                        </span>
                      )}
                    </td>
                    <td className="px-4 py-3 text-right">
                      <button
                        ...
                        className={`w-8 h-8 rounded-lg hover:bg-white/8 flex items-center justify-center ml-auto transition-colors disabled:opacity-40 ${
                          user.is_admin ? "text-gold hover:text-amber-300" : "text-crystal/30 hover:text-gold"
                        }`}
