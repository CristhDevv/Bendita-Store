  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-3xl text-charcoal mb-1">Marcas</h1>
          <p className="font-body text-sm text-charcoal-muted">{brands.length} marcas en total</p>
        </div>
        <button
          onClick={openCreate}
        ...
        {showForm && (
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block font-body text-xs text-charcoal-muted mb-1.5">País</label>
                <input
                  className={inputClass}
                  value={form.country}
                  onChange={(e) => setForm((f) => ({ ...f, country: e.target.value }))}
                  placeholder="Francia, España..."
                />
              </div>
              <div>
                <label className="block font-body text-xs text-charcoal-muted mb-1.5">URL del Logo</label>
                <input
                  className={inputClass}
                  value={form.logo_url}
              ...
              <div className="sm:col-span-2 flex gap-3 pt-1">
                ...
                >
                  ...
                  {saving ? "Guardando..." : editingId ? "Actualizar" : "Crear Marca"}
                </button>
              </div>
            </form>
          </motion.div>
      ...
      <div className="relative max-w-md">
        ...
        <input
          ...
          placeholder="Buscar por nombre, slug o país..."
          value={search}
          ...
          <div className="overflow-x-auto">
                ...
                {filtered.map((brand, i) => (
                  ...
                  >
                    <td className="px-4 py-3">
                      <div className="w-8 h-8 rounded-lg bg-cream border border-border flex items-center justify-center shrink-0 overflow-hidden">
                        {brand.logo_url ? (
                    ...
                    <td className="px-4 py-3">
                      <p className="font-body text-sm text-charcoal font-medium">{brand.name}</p>
                      {brand.description && (
                        <p className="font-body text-xs text-charcoal-muted line-clamp-1">{brand.description}</p>
                      )}
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        {brand.country && <Globe className="w-3 h-3 text-charcoal-muted" />}
                        <span className="font-body text-xs text-charcoal-muted">{brand.country || "—"}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3">
            ...
            {filtered.length === 0 && (
              <div className="py-12 text-center">
                <Tag className="w-8 h-8 text-gold/20 mx-auto mb-2" />
                <p className="font-body text-sm text-charcoal-muted">Sin marcas</p>
              </div>
            )}
          </div>
        </div>
