...
export function BrandShowcase() {
  ...
  return (
    <section className="py-24 bg-navy-900 border-y border-white/5 relative overflow-hidden glass">
      <div className="container mx-auto px-4 md:px-8 mb-12">
        <h2 className="font-display text-3xl md:text-4xl text-crystal text-center">
          Nuestras Marcas
        </h2>
      </div>
          ...
          <div className="flex">
            {BRANDS.map((brand, i) => (
              <div key={i} className="flex-[0_0_50%] md:flex-[0_0_25%] lg:flex-[0_0_16.666%] min-w-0 flex items-center justify-center px-4">
                <span className="font-display text-2xl md:text-3xl font-bold tracking-widest text-crystal/30 uppercase hover:text-gold transition-colors duration-300 cursor-pointer">
                  {brand}
                </span>
              </div>
            ))}
