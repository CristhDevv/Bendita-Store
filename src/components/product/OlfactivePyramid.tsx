...
export function OlfactivePyramid({ product }: { product: Product }) {
  ...
  return (
    <section className="py-20 bg-navy-950 border-t border-white/5 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-16">
          ...
          <h2 className="font-display text-4xl text-crystal">Pirámide Olfativa</h2>
        </div>

        <div className="max-w-2xl mx-auto flex flex-col gap-16 relative">
          {/* Vertical connection line */}
          <div className="absolute left-1/2 top-10 bottom-10 w-px bg-gradient-to-b from-transparent via-gold-500/30 to-transparent -translate-x-1/2 hidden md:block" />
          ...
          {product.notes_top && product.notes_top.length > 0 && (
            <div className="flex flex-col md:flex-row items-center justify-center gap-6 relative">
              <div className="md:w-1/3 text-center md:text-right">
                ...
                <p className="font-body text-xs text-crystal/50 uppercase tracking-wider">Los primeros 15 min</p>
              </div>
              <div className="w-16 h-16 shrink-0 rounded-full bg-navy-900 border border-gold-500/30 flex items-center justify-center z-10 shadow-[0_0_20px_rgba(201,162,39,0.1)]">
              ...
              <div className="md:w-1/3 flex flex-wrap justify-center md:justify-start gap-2">
                {product.notes_top.map(n => <span key={n} className="px-3 py-1.5 rounded-full bg-navy-800 text-crystal text-sm border border-white/5">{n}</span>)}
              </div>
            </div>
          )}
          ...
          {product.notes_heart && product.notes_heart.length > 0 && (
            <div className="flex flex-col md:flex-row items-center justify-center gap-6 relative">
              <div className="md:w-1/3 text-center md:text-right">
                ...
                <p className="font-body text-xs text-crystal/50 uppercase tracking-wider">De 15 min a 4 hrs</p>
              </div>
              <div className="w-16 h-16 shrink-0 rounded-full bg-navy-900 border border-gold-500/30 flex items-center justify-center z-10 shadow-[0_0_20px_rgba(201,162,39,0.1)]">
              ...
              <div className="md:w-1/3 flex flex-wrap justify-center md:justify-start gap-2">
                {product.notes_heart.map(n => <span key={n} className="px-3 py-1.5 rounded-full bg-navy-800 text-crystal text-sm border border-white/5">{n}</span>)}
              </div>
            </div>
          )}
          ...
          {product.notes_base && product.notes_base.length > 0 && (
            <div className="flex flex-col md:flex-row items-center justify-center gap-6 relative">
              <div className="md:w-1/3 text-center md:text-right">
                ...
                <p className="font-body text-xs text-crystal/50 uppercase tracking-wider">Hasta 24 horas</p>
              </div>
              <div className="w-16 h-16 shrink-0 rounded-full bg-navy-900 border border-gold-500/30 flex items-center justify-center z-10 shadow-[0_0_20px_rgba(201,162,39,0.1)]">
              ...
              <div className="md:w-1/3 flex flex-wrap justify-center md:justify-start gap-2">
                {product.notes_base.map(n => <span key={n} className="px-3 py-1.5 rounded-full bg-navy-800 text-crystal text-sm border border-white/5">{n}</span>)}
              </div>
            </div>
          )}
        </div>
